...
%         
%     else
%         if ~isfield(str, 'average_c2')
%             pre = phi <=0 ;
%             [~, str.average_c2, ~, ~] = compute_ventricle_ellipse_params(pre);
%         end
%         str.signum = str.k >= str.average_c2;
%     end
...
%     end
    ...
    A = str.DrcU .* str.g .* alpha + C ;
    L = Length(phi, str, param);
    P = Penalized(phi, str, param);
    phi = Numerical(phi, A, L, P, str, param);
    
    % compute image  (Img - str.Img)./(str.Img.^2 + eps) + + L_k
%     if its < 200
%     str.Img = str.Img + timestep2.*  ( lambda3.*((Img - str.Img)./(str.Img).^2) + str.k2 );
